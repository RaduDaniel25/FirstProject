...
const renderCrypto = function (data, crypto) {
  const html = `
...
export const getCryptoPrice2 = async function () {
  try {
    ...
    renderCrypto(data1.data[4].priceUsd, binance);
    renderCrypto(data1.data[28].priceUsd, near);
    renderCrypto(data1.data[11].priceUsd, dot);
    renderCrypto(data1.data[5].priceUsd, xrp);
  } catch (err) {
    console.log(err);
  }
};
...
export const getCryptoPrice3 = async function () {
  try {
    const data1 = await getJSON(`https://api.changelly.com`);
    console.log(data1);
  } catch (err) {
    console.log(err);
  }
